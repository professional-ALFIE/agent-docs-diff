...
| 설치된 플러그인                | `~/.claude/plugins` | 복제된 마켓플레이스, 설치된 플러그인 버전, 플러그인별 데이터로, `claude plugin` 명령으로 관리됩니다. 마켓플레이스 [`command` 소스](/docs/ko/plugin-marketplaces#command-sources)에서 링크 모드로 설치된 플러그인의 경우, Claude Code는 복사본 대신 여기에 링크를 저장하고, 플러그인의 파일은 명령이 출력하는 디렉터리에 남아 있습니다. 고아 버전이 정리되는 방식은 [플러그인 캐싱](/docs/ko/plugins-reference#plugin-caching-and-file-resolution)을 참조하세요. |
...
Claude Code는 [`cleanupPeriodDays`](/docs/ko/settings-reference#cleanupperioddays)보다 오래된 아래 경로의 파일을 삭제합니다. 보유 기간을 안전하게 결정할 수 있는 한 말입니다. 기본값은 30일이고 최소값은 1입니다. `0`으로 설정하면 유효성 검사 오류가 발생합니다. 동일한 나이 기준이 [고아 worktrees](/docs/ko/worktrees#clean-up-subagent-and-background-session-worktrees)의 자동 제거에 적용됩니다.
...
| `debug/`                                                                                                                        | 세션별 디버그 로그로, [`--debug`](/docs/ko/cli-reference#cli-flags)로 시작하거나 `/debug`를 실행할 때와 같이 디버그 로깅이 켜져 있을 때 작성됩니다.                                                                                                                 |
| `paste-cache/`                                                                                                                  | 대형 붙여넣기의 내용                                                                                                                                                                                                             |
| `image-cache/<session>/`                                                                                                        | 첨부된 이미지입니다. 각 스윕에서 Claude Code는 나이에 관계없이 다른 모든 세션의 디렉토리를 제거합니다.                                                                                                                                                         |
| `uploads/<session>/`                                                                                                            | [Remote Control](/docs/ko/remote-control) 세션에 메시지를 보낼 때 웹 또는 모바일 앱에서 첨부한 파일과 모바일 앱에서 첨부한 사진입니다. [cloud session](/docs/ko/claude-code-on-the-web)에 대한 첨부는 대신 해당 세션의 자체 클라우드 환경에 저장되며, 컴퓨터에는 저장되지 않습니다.                             |
| `session-env/`                                                                                                                  | 세션별 환경 메타데이터                                                                                                                                                                                                            |
| `tasks/`                                                                                                                        | 작업 도구로 작성된 작업 목록으로, 목록당 하나의 디렉토리                                                                                                                                                                                        |
...
| `backups/`                                                                                                                      | `~/.claude.json`의 이전 버전으로, Claude Code가 파일을 다시 쓸 때 복사됩니다. Claude Code는 가장 최신의 5개와 구문 분석할 수 없는 모든 버전의 복사본을 유지합니다.                                                                                                        |
| `feedback-bundles/`                                                                                                             | 제3자 공급자에서 `/feedback`으로 작성되거나 Anthropic 자격 증명이 구성되지 않은 경우 작성된 수정된 트랜스크립트 아카이브로, Anthropic 계정 팀에 전송하기 위함                                                                                                                 |
| `feedback/drafts/`                                                                                                              | `/feedback`에서 검토를 기다리는 [Claude-drafted feedback](/docs/ko/tools-reference#sendfeedback-tool-behavior)을 대기 중입니다. `cleanupPeriodDays` 또는 30일 중 더 짧은 기간 후에 스윕됩니다. 큐가 10개 초안 제한에 도달하면 Claude Code는 공간을 확보하기 위해 가장 오래된 초안을 삭제합니다. |
| `usage-data/`                                                                                                                   | [`/insights`](/docs/ko/costs#analyze-your-usage-patterns)로 작성된 `report.html` 및 타임스탬프 보고서 복사본과 이를 구축하는 데 사용되는 캐시된 세션별 분석 데이터                                                                                                  |
| `todos/`, `statsig/`, `logs/`                                                                                                   | 이전 버전의 레거시 디렉토리입니다. 더 이상 작성되지 않습니다. 스윕은 내용을 제거한 후 빈 디렉토리를 제거합니다.                                                                                                                                                        |

`sessions/`의 세션 파일, 자동 메모리, Claude Desktop 및 Cowork 트랜스크립트는 각각 자체 보유 규칙을 따릅니다:

* **`sessions/`**: 실행 중인 세션당 하나의 작은 파일을 보유하며, 동시 세션 및 충돌을 감지하는 데 사용됩니다. 나이 기반 스윕의 일부가 아닙니다: Claude Code는 세션이 종료될 때 각 파일을 제거하고 다음 시작 시 충돌 잔여물을 정리합니다.
* **자동 메모리**: 스윕은 프로젝트의 [auto memory](/docs/ko/memory#auto-memory) 디렉토리 `projects/<project>/memory/`의 메모리 파일을 삭제하지 않습니다. Claude Code는 전체 보유 기간 동안 비어 있었던 경우에만 해당 디렉토리를 제거합니다. v2.1.228 이전에는 스윕이 메모리 디렉토리 내의 폴더를 세션 데이터로 취급하고 그 아래의 오래된 파일을 삭제할 수 있었습니다.
* **Claude Desktop 및 Cowork 트랜스크립트**: Claude Code는 Claude Desktop 또는 Cowork에서 시작하거나 가장 최근에 계속한 세션의 트랜스크립트를 모든 나이에서 유지합니다. 이러한 트랜스크립트에 나이 제한을 부여하려면 [`desktopSessionCleanupPeriodDays`](/docs/ko/settings-reference#desktopsessioncleanupperioddays)를 설정합니다. [managed settings](/docs/ko/managed-settings)가 `cleanupPeriodDays`를 설정하면 Claude Code는 대신 해당 기간 후에 이러한 트랜스크립트를 삭제합니다. Claude Code v2.1.248 이상이 필요합니다. 이전 버전은 `cleanupPeriodDays` 후에 삭제합니다.

Claude Code는 다음 경우에 스윕을 완전히 건너뜁니다:

* **Bare mode**: [`--bare`](/docs/ko/headless#start-faster-with-bare-mode)로 `claude -p`를 실행할 때, Claude Code는 해당 세션에서 스윕을 실행하지 않습니다.
* **Paused sweep**: Claude Code가 보유 기간을 안전하게 결정할 수 없으면 보유 정리 스윕을 일시 중지합니다. [`retention_sweep` event](/docs/ko/monitoring-usage#retention-sweep-event)는 이를 일시 중지하는 각 구성을 나열합니다. 원인이 읽거나 구문 분석할 수 없는 설정 파일이거나 `cleanupPeriodDays` 또는 `desktopSessionCleanupPeriodDays`가 명시적으로 설정된 설정 오류인 경우, Claude Code는 설정 오류를 수정할 때까지 `/status`에 경고를 표시합니다. [managed settings](/docs/ko/server-managed-settings)가 `cleanupPeriodDays`를 제공하면 Claude Code는 어느 경우든 관리되는 값에서 스윕을 실행합니다.

<h3 id="kept-until-you-delete-them">
  삭제할 때까지 유지됨
</h3>

보유 정리 스윕은 아래 경로를 제거하지 않습니다. Claude Code는 삭제할 때까지 유지하며, 로그아웃할 때 삭제하는 두 개의 캐시는 제외합니다.

| `~/.claude/` 아래 경로     | 내용                                                                                                                                                                                                                                             |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `history.jsonl`        | 입력한 모든 프롬프트로, 타임스탬프 및 프로젝트 경로 포함. 위쪽 화살표 회상, `Ctrl+R` 기록 검색, `!` 셸 명령 완성에 사용됩니다.                                                                                                                                                               |
| `stats-cache.json`     | `/usage`로 표시된 집계 토큰 및 비용 계산                                                                                                                                                                                                                    |
| `remote-settings.json` | 조직의 [server-managed settings](/docs/ko/server-managed-settings)의 캐시된 복사본이거나, 조직이 아무것도 설정하지 않은 경우 `{}`입니다. 세션이 [fetches them](/docs/ko/server-managed-settings#platform-availability)할 때만 존재합니다. Claude Code는 시작 시 및 세션 중 매시간 업데이트를 확인합니다. 로그아웃할 때 삭제됩니다. |
| `cache/changelog.md`   | Claude Code changelog의 캐시된 복사본으로, `/release-notes`로 표시됩니다. 백그라운드에서 새로고침됩니다.                                                                                                                                                                    |
| `policy-limits.json`   | 조직의 캐시된 기능 정책 설정입니다. 일부 계정 유형에만 존재합니다. 자동으로 새로고침됩니다. 로그아웃할 때 삭제됩니다.                                                                                                                                                                            |

<span id="state-files-to-keep" />

기타 파일은 사용하는 기능에 따라 나타납니다. 캐시 및 잠금 파일은 안전하게 삭제할 수 있습니다. 이러한 상태 파일을 유지합니다:

* `.credentials.json`: 귀하의 [login credentials](/docs/ko/authentication#credential-management)
* `agent-memory/`: [subagent memory](/docs/ko/sub-agents#enable-persistent-memory)
...
* [`desktopSessionCleanupPeriodDays`](/docs/ko/settings-reference#desktopsessioncleanupperioddays)를 설정하여 Claude Desktop 및 Cowork 트랜스크립트에 나이 제한을 부여합니다.
* [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`](/docs/ko/env-vars) 환경 변수를 설정하여 모든 모드에서 트랜스크립트 및 프롬프트 기록 작성을 건너뜁니다. 비대화형 모드에서는 대신 `-p`와 함께 `--no-session-persistence`를 전달하거나 TypeScript Agent SDK에서 `persistSession: false`를 설정할 수 있습니다. Python SDK에는 동등한 옵션이 없습니다.
...
`claude project purge`를 실행하여 한 프로젝트에 대해 Claude Code가 보유한 상태를 삭제합니다. 다음을 삭제합니다:
...
아래 예제는 `~/work/my-repo`를 자리 표시자로 사용합니다. 프로젝트의 경로로 바꿉니다. 경로와 일치하는 상태가 없으면 명령은 오류를 인쇄하고 상태 1로 종료됩니다.
...
이 명령은 동일한 계획을 인쇄한 후 `Delete 3 item(s) for /home/user/work/my-repo? This cannot be undone. [y/N]`을 묻고 `y`로 답할 때만 삭제합니다.
...
이 명령은 프로젝트 범위가 아니므로 `shell-snapshots/` 및 `backups/`는 그대로 두고 계획 출력에서 이에 대해 경고합니다.

위의 애플리케이션 데이터 경로를 언제든지 손으로 삭제할 수 있습니다. [state files to keep](#state-files-to-keep) 제외. 새 세션은 영향을 받지 않습니다. 아래 표는 과거 세션에서 손실되는 항목을 보여줍니다.

| 삭제                                                                                                                                             | 손실 항목                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `~/.claude/projects/`                                                                                                                          | 과거 세션의 재개, 계속, 되감기, 모든 프로젝트의 자동 메모리                                                     |
| `~/.claude/history.jsonl`                                                                                                                      | 위쪽 화살표 프롬프트 회상, `Ctrl+R` 기록 검색, `!` 셸 명령 완성                                             |
| `~/.claude/paste-cache/`                                                                                                                       | 회상된 프롬프트의 붙여넣은 텍스트. [paste large content](/docs/ko/terminal-config#paste-large-content) 참조   |
...
| `~/.claude/feedback-bundles/`                                                                                                                  | Anthropic 계정 팀에 아직 전송하지 않은 피드백 및 버그 보고서 아카이브                                            |
| `~/.claude/feedback/drafts/`                                                                                                                   | 전송하지 않은 [Claude-drafted feedback](/docs/ko/tools-reference#sendfeedback-tool-behavior)       |
| `~/.claude/remote-settings.json`                                                                                                               | 없음. 다음 시작 시 다시 가져옵니다.                                                                   |
| `~/.claude/cache/changelog.md`                                                                                                                 | 없음. 백그라운드에서 새로고침됩니다.                                                                    |
| `~/.claude/policy-limits.json`                                                                                                                 | 없음. 자동으로 새로고침됩니다.                                                                       |
| `~/.claude/tasks/`                                                                                                                             | 재개된 세션이 선택할 작업 목록                                                                       |
| `~/.claude/debug/`, `~/.claude/plans/`, `~/.claude/image-cache/`, `~/.claude/session-env/`, `~/.claude/shell-snapshots/`, `~/.claude/backups/` | 사용자 대면 항목 없음                                                                            |

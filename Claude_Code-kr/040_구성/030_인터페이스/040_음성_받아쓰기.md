> 원본: https://code.claude.com/docs/ko/voice-dictation.md
...
음성 받아쓰기가 활성화되어 있는 동안 처음 세 세션 동안 입력 바닥글은 프롬프트가 비어 있을 때 `hold space to speak` 힌트를 표시합니다. 힌트는 현재 `voice:pushToTalk` 바인딩을 반영하며, [받아쓰기 키를 다시 바인딩](#rebind-the-dictation-key)하면 업데이트됩니다. 힌트 텍스트는 두 모드 모두에서 동일하며, [사용자 정의 상태 줄](/docs/ko/statusline)이 구성된 경우 나타나지 않습니다.
...
`Space`를 누르고 있어서 녹음을 시작합니다. Claude Code는 터미널에서 빠른 키 반복 이벤트를 감시하여 누르고 있는 키를 감지하므로 녹음이 시작되기 전에 짧은 워밍업이 있습니다. 바닥글은 워밍업 중에 `keep holding…`을 표시한 다음 녹음이 활성화되면 `listening…`으로 표시됩니다. 녹음 중에 프롬프트 커서는 마이크 레벨에 따라 올라갔다 내려가는 막대가 되며, [`prefersReducedMotion`](/docs/ko/settings-reference#prefersreducedmotion)을 켜지 않은 경우입니다.
...
`/voice tap`으로 탭 모드를 활성화합니다. 프롬프트 입력이 비어 있으면 `Space`를 탭하여 녹음을 시작합니다. 바닥글은 녹음 중에 `● REC · tap to send`를 표시합니다. `Space`를 다시 탭하여 중지합니다.

Claude Code는 전사를 삽입하고 전사가 최소 3단어 이상이면 프롬프트를 자동으로 제출합니다. 더 짧은 전사는 삽입되지만 제출되지 않으므로 실수로 탭해도 단어가 전송되지 않습니다.

3단어 임계값은 공백 없이 작성된 언어의 단어를 계산합니다. 일본어, 중국어, 태국어 전사는 개별 단어를 계산하므로 탭 모드와 `autoSubmit`이 있는 홀드 모드에서 자동으로 제출됩니다.
...
음성 받아쓰기는 Claude의 응답 언어를 제어하는 동일한 [`language` 설정](/docs/ko/settings-reference#language)을 사용합니다. 해당 설정이 비어 있으면 받아쓰기는 기본적으로 영어입니다. VS Code 확장에서 `language`가 비어 있으면 받아쓰기는 VS Code의 `accessibility.voice.speechLanguage` 설정을 사용한 후 기본적으로 영어로 설정됩니다.
...
* **`Voice mode is disabled by your organization's policy`**: 조직의 관리자 정책이 음성 받아쓰기를 비활성화합니다. 조직 관리자에게 연락하여 조직에서 음성 받아쓰기를 사용할 수 있는지 확인하세요.
...
* **Linux에서 `Voice mode requires SoX for audio recording`**: 네이티브 오디오 모듈을 로드할 수 없고 폴백이 설치되지 않았습니다. 오류 메시지에 표시된 명령으로 SoX를 설치하세요. 예: `sudo apt-get install sox`.
...
* **`Voice stream error: WebSocket upgrade rejected with HTTP <status>`**: 서버가 표시된 HTTP 상태로 연결을 거부했으므로 이는 네트워크 중단이 아닙니다. 400 범위의 상태는 일반적으로 만료된 로그인, 프록시 또는 전사 서비스 대신 응답하는 봇 보호 서비스를 의미합니다. `/login`을 실행하여 로그인을 새로 고치고, 상태가 지속되면 네트워크 경로에서 VPN 또는 프록시를 확인하세요. 거부가 도착할 때 여전히 녹음 중이면 Claude Code는 400 범위 외의 상태를 한 번 재시도한 후 이 메시지를 표시합니다. 400 범위의 상태는 재시도하지 않습니다. v2.1.229부터 v2.1.231에서는 네이티브 빌드가 이 메시지를 표시하지 않았습니다. Claude Code는 계속 녹음했고, 누르고 있기 모드 바닥글은 여전히 `listening…`을 표시했으며, 녹음을 중지한 후 `Voice connection failed`를 보고했습니다.
...
* [모든 설정](/docs/ko/settings-reference#voice): `voice`, `language` 및 기타 설정 키

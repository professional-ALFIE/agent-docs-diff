> 원본: https://code.claude.com/docs/ko/corporate-launcher.md
...
* [에이전트 팀](/docs/ko/agent-teams)이 tmux 또는 iTerm2에서 시작하는 분할 창 팀원 세션입니다. 팀원 창은 백그라운드 프로세스가 아닌 대화형이지만, Claude Code는 자신의 바이너리에서 이들을 시작하므로 런처가 이들을 포함합니다. Claude Code v2.1.210 이상이 필요합니다.
...
다음 프로세스는 런처를 통해 시작되지 않습니다:

* [설치된 백그라운드 서비스](/docs/ko/agent-view#the-supervisor-process): 런처가 구성되기 전에 작성된 단위를 가진 경우, `launchd` 또는 `systemd`가 해당 프로세스를 단위 파일에서 시작합니다. `/status`와 `claude daemon status`는 실행 중인 서비스와 구성된 런처가 일치하지 않을 때 경고하며, 서비스가 변수를 설정한 상태로 재시작되면 서비스가 생성하는 세션은 여전히 런처를 통해 시작됩니다.
...
* `--worktree`와 `--tmux`를 함께 수행하는 재시작: 터미널 멀티플렉서가 해당 창을 시작하며, Claude Code의 바이너리가 아닙니다.
* [Chrome의 Claude](/docs/ko/chrome)가 등록하는 네이티브 메시징 호스트: 브라우저가 이를 시작하며, Claude Code의 바이너리가 아닙니다.

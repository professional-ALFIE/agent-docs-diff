> 원본: https://code.claude.com/docs/ko/statusline.md
...
상태 표시줄은 기본 제공 바닥글 배지 위의 자체 행에서 렌더링되며 이를 대체하지 않습니다. 사용자 정의 상태 표시줄이 구성되면 Claude Code는 `esc to interrupt`, `? for shortcuts` 폴백 및 `hold space to speak` [음성 받아쓰기](/docs/ko/voice-dictation) 힌트를 포함한 바닥글의 대부분의 키보드 힌트 표시를 중단합니다. 대화에 ID가 나타날 때 스크립트를 작성하지 않고 바닥글에 클릭 가능한 링크 배지를 추가하려면 대신 [`footerLinksRegexes`](/docs/ko/settings-reference#footerlinksregexes)를 구성합니다.
...
설정 중에 Claude Code가 파일 편집 프롬프트에 대한 권한을 요청하면 승인합니다.

<h3 id="manually-configure-a-status-line">
  상태 표시줄 수동 구성
</h3>

사용자 설정(`~/.claude/settings.json`, 여기서 `~`는 홈 디렉토리) 또는 [프로젝트 설정](/docs/ko/settings#where-settings-live)에 `statusLine` 필드를 추가합니다. `type`을 `"command"`로 설정하고 `command`를 스크립트 경로 또는 인라인 셸 명령으로 지정합니다. 스크립트 생성에 대한 전체 설명은 [상태 표시줄 단계별 구축](#build-a-status-line-step-by-step)을 참조하세요.
...
<Steps>
  <Step title="JSON을 읽고 출력을 인쇄하는 스크립트 만들기">
    Claude Code는 stdin을 통해 JSON 데이터를 스크립트로 보냅니다. 이 스크립트는 [`jq`](https://jqlang.org/)(설치해야 할 수 있는 명령줄 JSON 파서)를 사용하여 모델 이름, 디렉토리 및 컨텍스트 백분율을 추출한 다음 형식이 지정된 줄을 인쇄합니다.
  ...
  <Step title="설정에 추가">
    ...
    상태 표시줄이 인터페이스 하단에 나타납니다. Claude Code는 설정을 자동으로 다시 로드하고 파일을 저장하는 즉시 스크립트를 실행합니다.
  </Step>
</Steps>
...
Claude Code는 스크립트를 [JSON 세션 데이터](#available-data)와 함께 stdin에서 실행하고 스크립트가 stdout에 인쇄하는 모든 것을 표시합니다.

**업데이트 시기**

스크립트는 세션이 시작될 때(재개할 때 포함) 한 번 실행됩니다. 그 후에는 다음 경우에 다시 실행됩니다:

* 새로운 어시스턴트 메시지가 도착할 때
* `/compact`가 완료될 때
* 권한 모드가 변경될 때
* Vim 모드가 전환될 때
* `statusLine` 설정에서 `command`를 변경할 때
* [`refreshInterval`](#manually-configure-a-status-line) 타이머가 경과할 때(설정한 경우)
* 스크립트가 마지막으로 받은 데이터의 [rate-limit 윈도우](#rate-limit-usage)가 `resets_at` 시간에 도달할 때
* 스크립트가 마지막으로 받은 데이터의 warm [prompt cache](#prompt-cache-fields)가 `expires_at` 시간에 도달할 때

Claude Code는 업데이트를 300ms에서 디바운스하므로 빠른 변경이 함께 일괄 처리되고 스크립트는 변경이 멈춘 후 한 번 실행됩니다. `command` 자체에 대한 변경은 디바운스를 건너뜁니다: Claude Code는 새 명령을 즉시 실행합니다. 스크립트가 여전히 실행 중인 동안 새 업데이트가 트리거되면 Claude Code는 진행 중인 스크립트를 취소합니다. 스크립트를 편집하면 업데이트 트리거가 다시 실행될 때 변경 사항이 나타납니다.

이벤트 기반 트리거는 주 세션이 유휴 상태일 때(예: 코디네이터가 백그라운드 서브에이전트를 기다릴 때) 조용해질 수 있습니다. 유휴 기간 동안 시간 기반 또는 외부 소스 세그먼트를 최신 상태로 유지하려면 [`refreshInterval`](#manually-configure-a-status-line)을 설정하여 고정 타이머에서도 명령을 다시 실행합니다.
...
Claude Code는 스크립트의 출력을 캡처하므로 터미널에 직접 연결하지 않아 스크립트 내부에서 `tput cols`와 언어 수준의 너비 감지가 터미널 크기를 읽을 수 없습니다. `COLUMNS` 및 `LINES` 환경 변수를 대신 읽으세요. Claude Code는 스크립트를 실행하기 전에 이러한 변수를 현재 터미널 크기로 설정합니다.
...
| `workspace.git_worktree`                                                         | `git worktree add`로 생성된 연결된 worktree 내에 현재 디렉토리가 있을 때 Git worktree 이름. 주 작업 트리에는 없습니다. `worktree.*`와 달리 모든 git worktree에 대해 채워지며, [worktree 세션](/docs/ko/worktrees) 중에만 존재하는 것이 아닙니다.                                                                                                         |
| `workspace.repo.host`, `workspace.repo.owner`, `workspace.repo.name`             | `origin` 원격에서 파싱된 저장소 식별자(예: `"github.com"`, `"anthropics"`, `"claude-code"`). git 저장소 외부에 있거나 `origin` 원격이 구성되지 않은 경우 없음. gitlab.com 프로젝트가 하위 그룹에 중첩된 경우 `owner`는 슬래시가 있는 전체 네임스페이스 경로(예: `"group/subgroup"`)입니다. v2.1.260 이전에는 이러한 프로젝트에 대해 `workspace.repo`가 없었습니다                    |
| `cost.total_cost_usd`                                                            | USD 단위의 예상 세션 비용으로, [`modelPricing`](/docs/ko/settings-reference#modelpricing) 테이블이 적용되지 않는 한 클라이언트 측에서 정가로 계산됩니다. 실제 청구서와 다를 수 있습니다. `/clear`가 새 세션을 시작할 때 \$0으로 재설정됩니다. v2.1.211 이전에는 `/clear` 후에도 합계가 유지되었습니다                                                                              |
...
| `context_window.total_input_tokens`, `context_window.total_output_tokens`        | 컨텍스트 윈도우에 현재 있는 토큰 수(가장 최근 API 응답에서). 입력에는 캐시 읽기 및 쓰기가 포함됩니다                                                                                                                                                                                                                             |
...
| `fast_mode`                                                                      | 세션에 대해 [빠른 모드](/docs/ko/fast-mode)가 활성화되어 있는지 여부                                                                                                                                                                                                                                              |
...
| `rate_limits.spend_limit.used_percentage`, `rate_limits.spend_limit.resets_at`   | [Claude 앱 게이트웨이](/docs/ko/claude-apps-gateway-spend-limits#usage-warnings-in-claude-code) 뒤에서 사용자에게 적용되는 지출 제한의 사용된 백분율 및 해당 기간이 재설정되는 Unix epoch 초. 백분율은 0\~100 범위이거나 제한을 초과하면 100 이상입니다. Claude Code v2.1.251 이상 필요                                                                         |
| `prompt_cache`                                                                   | 주 대화의 세션 [프롬프트 캐시](/docs/ko/prompt-caching) 통계: 히트율, 미스 및 캐시가 따뜻한지 여부. 모든 필드는 [프롬프트 캐시 필드](#prompt-cache-fields)를 참조하세요. 주 대화의 첫 번째 API 응답까지 없습니다. Claude Code v2.1.251 이상 필요                                                                                                                 |
| `session_id`                                                                     | 고유 세션 식별자                                                                                                                                                                                                                                                                                |
| `session_name`                                                                   | 세션 이름. `--name` 플래그 또는 `/rename`으로 설정된 사용자 정의 이름이 있을 때 사용하고, 그렇지 않으면 AI가 생성한 세션 제목을 사용합니다. [기본 표시 이름](/docs/ko/sessions#name-your-sessions)(예: `my-app-3f`)은 이 필드를 채우지 않습니다. 세션에 사용자 정의 이름도 AI가 생성한 제목도 없을 때는 없습니다                                                                            |
| `prompt_id`                                                                      | 현재 처리 중인 사용자 프롬프트를 식별하는 UUID. OpenTelemetry 이벤트의 [`prompt.id` 속성](/docs/ko/monitoring-usage#event-correlation-attributes)과 일치합니다. 첫 번째 사용자 입력까지 없습니다. Claude Code v2.1.196 이상 필요                                                                                                              |
...
| `pr.number`, `pr.url`                                                            | 현재 브랜치에 대한 열린 풀 요청. 하단의 PR 배지를 반영합니다. GitLab 원격이 있는 저장소에서 Claude Code는 대신 브랜치의 열린 [병합 요청](/docs/ko/interactive-mode#gitlab-merge-requests)에서 이 필드를 채우므로 `pr.number`는 병합 요청 번호입니다. 병합 요청 데이터는 Claude Code v2.1.234 이상 필요합니다. git 저장소에 없을 때, 풀 요청 또는 병합 요청을 찾을 때까지, 또는 병합되거나 닫힌 후에는 없습니다        |
| `pr.review_state`                                                                | 열린 PR의 검토 상태: `approved`, `pending`, `changes_requested` 또는 `draft`. `pr`이 있을 때도 독립적으로 없을 수 있습니다                                                                                                                                                                                         |
| `pr.kind`                                                                        | `pr`이 [GitLab 병합 요청](/docs/ko/interactive-mode#gitlab-merge-requests)을 설명할 때 `mr`. GitHub 풀 요청의 경우 없으므로 이 필드 이전에 작성된 스크립트는 계속 작동합니다. 병합 요청의 경우 Claude Code는 GitLab이 병합 가능으로 보고할 때 `review_state`를 `approved`로, 다른 모든 열린 상태에 대해 `pending`으로, 초안에 대해 `draft`로 설정합니다. Claude Code v2.1.234 이상 필요 |
| `worktree.name`                                                                  | 활성 worktree의 이름. [worktree 세션](/docs/ko/worktrees) 중에만 표시됩니다                                                                                                                                                                                                                                  |
| `worktree.path`                                                                  | worktree 디렉토리의 절대 경로                                                                                                                                                                                                                                                                     |
| `worktree.branch`                                                                | worktree의 Git 브랜치 이름(예: `"worktree-my-feature"`). 훅 기반 worktree의 경우 없습니다                                                                                                                                                                                                                 |
| `worktree.original_cwd`                                                          | worktree에 들어가기 전에 Claude가 있던 디렉토리                                                                                                                                                                                                                                                        |
| `worktree.original_branch`                                                       | worktree에 들어가기 전에 체크아웃된 Git 브랜치. 훅 기반 worktree의 경우 없습니다                                                                                                                                                                                                                                  |
...
<Accordion title="전체 JSON 스키마">
  ...
  * `session_name`: `--name` 또는 `/rename`으로 사용자 정의 이름이 설정되었을 때 또는 AI가 생성한 세션 제목이 존재할 때 나타남. `my-app-3f`와 같은 기본 표시 이름은 이 필드를 채우지 않습니다
  ...
  * `pr`: 현재 브랜치에 대해 열린 PR 또는 GitLab 병합 요청을 찾았을 때만 나타나며, 병합되거나 닫히면 제거됩니다. `pr.review_state` 및 `pr.kind`는 독립적으로 없을 수 있습니다
  * `worktree`: [worktree 세션](/docs/ko/worktrees) 중에만 나타남. 존재할 때 `branch` 및 `original_branch`도 훅 기반 worktree의 경우 없을 수 있습니다
  * `rate_limits`: Claude.ai Pro 및 Max 구독자의 경우 또는 지출 제한을 설정하는 Claude 앱 게이트웨이 뒤에서만 나타나며, 세션의 첫 번째 API 응답 후에만 나타남. 각 윈도우(`five_hour`, `seven_day`, `spend_limit`)는 독립적으로 없을 수 있으며, Claude Code는 `resets_at` 시간이 지나면 윈도우를 삭제합니다. 부재를 우아하게 처리하려면 `jq -r '.rate_limits.five_hour.used_percentage // empty'`를 사용합니다.
  * `prompt_cache`: 주 대화의 첫 번째 API 응답 후에 나타남. [프롬프트 캐시 필드](#prompt-cache-fields) 참조
...
`context_window` 객체는 가장 최근 API 응답의 라이브 컨텍스트 윈도우를 설명합니다.
...
`prompt_cache` 객체는 세션의 주 대화가 [프롬프트 캐시](/docs/ko/prompt-caching)를 사용하는 방식을 요약합니다. Claude Code는 API의 응답에서 캐시 토큰 수를 계산하므로 모든 제공자에서 작동합니다.

이 객체는 주 대화의 첫 번째 API 응답 후에 나타납니다. Claude Code는 이 통계에서 하위 에이전트 요청을 계산하지 않습니다. Claude Code v2.1.251 이상 필요합니다.

표는 각 필드와 그 의미를 나열합니다. 타임스탬프는 Unix epoch 초이며, `rate_limits.*.resets_at`과 동일한 단위입니다. 짧은 상태 표시줄은 일반적으로 이 중 하나 또는 두 개를 표시합니다. `warm` 및 `hit_ratio`는 캐시 상태를 가장 직접적으로 요약합니다.

| 필드                       | 설명                                                                                                            |
| ------------------------ | ------------------------------------------------------------------------------------------------------------- |
| `warm`                   | 캐시된 접두사가 여전히 TTL 내에 있는지 여부. 마지막 응답이 캐시 토큰을 보고하지 않았을 때 `false`이지만 `caching_observed`는 `true`입니다                |
| `caching_observed`       | 이 세션의 응답이 캐시 토큰을 보고했는지 여부. `false`는 프롬프트 캐싱이 꺼져 있거나 제공자 또는 게이트웨이가 이를 보고하지 않음을 의미합니다                           |
| `ttl`                    | 현재 캐시된 접두사의 [캐시 수명](/docs/ko/prompt-caching#cache-lifetime): `"5m"` 또는 `"1h"`                                      |
| `expires_at`             | 캐시된 접두사가 TTL을 벗어나 콜드 상태가 되는 시간(epoch 초). 마지막 응답이 캐시 토큰을 보고하지 않았을 때 `null`                                     |
| `requests`               | 이 세션의 주 대화에 대해 기록된 API 요청                                                                                     |
| `misses`                 | 캐시가 이미 보유한 콘텐츠를 다시 처리한 요청: 5% 이상이고 캐시에서 읽을 수 있었던 최소 2,000개 토큰이며, 캐시 읽기의 부족을 설명할 압축 또는 도구 결과 지우기가 없습니다         |
| `expected_rebuilds`      | 압축 또는 이전 도구 결과 지우기 후에 따라온 캐시 재구축                                                                              |
| `hit_ratio`              | 이 세션의 모든 입력 토큰의 분수로서의 캐시 읽기 토큰(0\~1). 분모는 캐시 읽기, 캐시 쓰기 및 캐시되지 않은 입력을 계산합니다. 이러한 수가 모두 0일 때 `null`             |
| `cache_write_tokens`     | 이 세션에서 캐시에 기록된 모든 토큰(첫 번째 요청의 초기 쓰기 포함)                                                                       |
| `miss_recache_tokens`    | 미스로 계산된 요청에 의해 캐시에 기록된 토큰                                                                                     |
| `last_miss_at`           | 마지막 미스가 발생한 시간(epoch 초). 세션에 미스가 없을 때 `null`                                                                  |
| `last_miss_cause`        | Claude Code가 마지막 미스의 가능한 원인으로 식별한 것([마지막 미스 원인](#last-miss-cause) 아래에 설명됨). Claude Code v2.1.260 이상 필요        |
| `miss_causes`            | 이 세션의 진단된 미스 중 각 원인을 가진 개수(`last_miss_cause`와 동일한 원인 이름으로 키됨). Claude Code v2.1.260 이상 필요                     |
| `recache_tokens_if_cold` | 캐시가 그때까지 콜드 상태가 되었을 경우 다음 요청이 다시 캐시하는 토큰. 압축 또는 이전 도구 결과 지우기 직후 `null`이며, 다음 요청이 다시 작성된 대화의 크기를 기록할 때까지 유지됩니다 |

Claude Code는 터미널에서 동일한 통계를 표시하며, [`/usage` 명령의 `Prompt cache (main)` 줄](/docs/ko/costs#prompt-cache-statistics)에 표시됩니다.

<h4 id="last-miss-cause">
  마지막 미스 원인
</h4>

`last_miss_cause` 객체는 Claude Code가 가장 최근 미스의 가능한 원인으로 식별한 것을 보고합니다. 해당 `causes` 배열은 `tools_changed`, `system_prompt_changed`, `ttl_expired_5m` 또는 `likely_server_side`와 같은 하나 이상의 원인 이름을 보유합니다. 이 객체는 세션의 첫 번째 미스까지 `null`이며, Claude Code가 가장 최근 미스의 원인을 식별할 수 없을 때 다시 `null`입니다. Claude Code v2.1.260 이상 필요합니다.

두 원인은 객체에 수를 추가합니다:

* `tools_added` 및 `tools_removed`: `tools_changed`와 함께, 요청에 추가되거나 제거된 도구의 수
* `system_char_delta`: `system_prompt_changed`와 함께, 시스템 프롬프트의 길이 변화(문자 단위)
...
Bash 예제는 [`jq`](https://jqlang.org/)를 사용하여 JSON을 구문 분석합니다. Python 및 Node.js는 기본 제공 JSON 구문 분석을 가집니다.
...
스크립트는 여러 줄을 출력하여 더 풍부한 디스플레이를 만들 수 있습니다.
...
이 예제는 GitHub 저장소에 대한 클릭 가능한 링크를 만듭니다. Cmd(macOS) 또는 Ctrl(Windows/Linux)을 누르고 클릭하여 브라우저에서 링크를 엽니다.
...
Claude.ai 구독 속도 제한 사용량을 상태 표시줄에 표시합니다. `rate_limits` 객체에는 롤링 `five_hour` 윈도우와 주간 `seven_day` 윈도우가 포함됩니다. 각 윈도우는 `used_percentage`(0\~100) 및 `resets_at`(윈도우가 재설정되는 Unix epoch 초)를 제공합니다.

Claude 앱 게이트웨이 뒤에서 지출 제한이 있는 경우, `rate_limits`는 지출 제한에 대해 동일한 두 필드를 가진 `spend_limit`을 포함하며, 제한을 초과하면 `used_percentage`가 100을 초과할 수 있습니다. Claude Code v2.1.251 이상이 필요합니다.

`rate_limits` 객체는 Claude.ai Pro 및 Max 구독자 또는 지출 제한이 있는 Claude 앱 게이트웨이 뒤에서만 나타나며, 첫 번째 API 응답 이후에만 나타납니다. 각 스크립트는 부재한 필드를 우아하게 처리합니다:
...
<CodeGroup>
  ...
  cache_is_stale() {
      [ ! -f "$CACHE_FILE" ] || \
      # stat -c %Y (Linux) 또는 stat -f %m (macOS)는 파일의 마지막 수정 시간을 출력합니다.
      # Linux 형식이 먼저 실행되어야 합니다: Linux에서 macOS 형식은 실패하기 전에 stdout에 파일 시스템 보고서를 출력하고,
      # 해당 출력은 명령 대체로 캡처되어 산술을 중단합니다.
...
명령은 새로 고침 틱마다 한 번 실행되며 모든 표시 가능한 서브에이전트 행이 stdin의 단일 JSON 객체로 전달됩니다. 입력에는 [기본 훅 필드](/docs/ko/hooks#common-input-fields), `columns` 필드(사용 가능한 행 너비 포함) 및 `tasks` 배열이 포함됩니다. 각 작업에는 `id`, `name`, `type`, `status`, `description`, `label`, `startTime`, `model`, `effort`, `contextWindowSize`, `tokenCount`, `tokenSamples` 및 `cwd`가 있습니다.
...
작업별 `effort` 필드는 해당 서브에이전트에 대해 설정된 추론 노력으로, [정의 frontmatter](/docs/ko/sub-agents#supported-frontmatter-fields) 또는 개별 호출에서 설정됩니다. 값은 노력 수준 문자열 `low`, `medium`, `high`, `xhigh` 또는 `max` 중 하나이거나 숫자 토큰 예산입니다. 필드는 구성된 값을 작성된 그대로 보고합니다. 모델이 해당 수준을 지원하지 않으면 Claude Code가 실제로 적용하는 노력이 다를 수 있습니다. 필드는 Claude Code v2.1.214 이상이 필요하며 서브에이전트가 세션의 노력 수준을 상속할 때는 없습니다.
...
`statusLine`에 적용되는 동일한 신뢰, `disableAllHooks` 및 [`allowManagedHooksOnly`](/docs/ko/settings-reference#allowmanagedhooksonly) 게이트가 여기에 적용됩니다. 플러그인은 [`settings.json`](/docs/ko/plugins-reference#standard-plugin-layout)에서 기본 `subagentStatusLine`을 제공할 수 있지만, 훅과 달리 플러그인이 관리 설정 `enabledPlugins`에서 강제 활성화되어 있을 때도 플러그인 값은 `allowManagedHooksOnly` 아래에서 실행되지 않습니다.
...
* [설정 우선순위](/docs/ko/hooks#disable-or-remove-hooks)가 적용된 후 관리되는 설정 외부에서 `disableAllHooks`가 `true`인 경우, Claude Code는 관리되는 설정의 `statusLine`만 실행하며, 관리되는 `statusLine`이 없으면 상태 표시줄이 비활성화됩니다. 설정을 제거하거나 이를 설정하는 파일에서 `false`로 설정하여 다시 활성화합니다. [`disableAllHooks`](/docs/ko/settings-reference#disableallhooks)를 참조합니다.
* 조직에서 관리되는 설정에 `allowManagedHooksOnly`를 설정하면 사용자 정의 상태 표시줄이 경고 없이 사라집니다. 관리되는 설정의 `statusLine` 값에서만 상태 표시줄을 가져올 수 있습니다. [`allowManagedHooksOnly`에서 실행되는 항목](/docs/ko/settings-reference#what-runs-under-allowmanagedhooksonly)에서 전체 동작을 확인하고 이 설정이 사용자에게 적용되는지 관리자에게 문의합니다.
...
* 링크 텍스트가 나타나지만 클릭할 수 없으면 Claude Code가 터미널에서 하이퍼링크 지원을 감지하지 못했을 수 있습니다. Claude Code를 시작하기 전에 `FORCE_HYPERLINK` 환경 변수를 설정하여 감지를 재정의합니다:
...
* `statusLine`이 셸 명령을 실행하므로 Claude Code는 [설정 파일의 훅과 동일한 워크스페이스 신뢰 규칙](/docs/ko/permissions#what-runs-before-you-trust-a-folder)에서 실행합니다. 폴더에 대한 대화를 수락하거나 신뢰가 이를 확장하는 상위 디렉토리를 수락하면 충분합니다.
* 그때까지 상태 표시줄은 공백으로 유지되며, `claude --debug`는 `Status line command skipped: workspace trust not accepted`를 기록합니다. Claude Code를 다시 시작하고 신뢰 대화를 수락하여 활성화합니다.
...
[전체 화면 렌더링](/docs/ko/fullscreen) 외부에서 Claude Code는 상태 표시줄과 동일한 행에 알림을 표시합니다. 전체 화면 렌더링에서 Claude Code는 알림에 자체 행을 제공합니다.

* MCP 서버 오류 및 자동 업데이트와 같은 시스템 알림은 행의 오른쪽에 표시됩니다. 컨텍스트 부족 경고와 같은 일시적 알림도 이 영역을 순환합니다.

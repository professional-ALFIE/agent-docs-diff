> 원본: https://code.claude.com/docs/ko/desktop-scheduled-tasks.md
...
Desktop 앱의 **Routines** 페이지를 통해 로컬 예약된 작업과 원격 [routines](/docs/ko/routines)을 모두 만들 수 있습니다. 로컬 작업은 사용자의 머신에서 실행되며 파일과 도구에 직접 접근할 수 있지만, 앱이 열려 있고 컴퓨터가 깨어 있을 때만 실행됩니다. 원격 routine은 컴퓨터가 꺼져 있어도 클라우드에서 실행되며, API 호출이나 GitHub 이벤트에 대해서도 실행될 수 있습니다. 이 페이지는 로컬 예약된 작업을 다룹니다. 원격 routine과 해당 트리거 옵션에 대해서는 [Routines](/docs/ko/routines)을 참조하세요.
...
Claude Desktop 1.1.5368 이전 버전에서는 로컬 예약된 작업을 사용할 수 없습니다. [**Code** 탭](/docs/ko/desktop)에서 사이드바의 **Routines**을 클릭하거나 사이드바의 **More** 메뉴에서 클릭한 다음 **New routine**을 클릭하고 **Local**을 선택합니다. 다음 필드를 구성합니다:
...
작업이 실행되면 데스크톱 알림을 받고 새 세션이 사이드바의 **Scheduled** 섹션 아래에 나타납니다. 이를 열어 Claude가 수행한 작업을 확인하고, 변경 사항을 검토하거나, 권한 프롬프트에 응답합니다. Claude는 파일을 편집하고, 명령을 실행하고, 커밋을 만들고, 풀 요청을 열 수 있으며, 사용자가 직접 시작한 세션과 동일하게 작동하지만, desktop 앱의 세션 표면을 통해 [데스크톱 세션 간 메시지](/docs/ko/desktop#work-across-sessions)를 주고받을 수 없습니다.
...
각 작업에는 자신의 권한 모드가 있으며, 이는 작업을 만들거나 편집할 때 설정합니다. `~/.claude/settings.json`의 Allow 규칙도 예약된 작업 세션에 적용됩니다. 작업이 [Manual 모드](/docs/ko/desktop#choose-a-permission-mode)에서 실행되고 권한이 없는 도구를 실행해야 하는 경우, 승인할 때까지 실행이 정지됩니다. 세션은 사이드바에 열려 있으므로 나중에 답변할 수 있습니다.
...
[`requiresUserInteraction`](/docs/ko/mcp#require-approval-for-a-specific-tool)으로 표시된 MCP 도구는 매번 호출할 때마다 프롬프트를 표시하며 always-allow 옵션을 제공하지 않습니다. 이러한 도구를 호출하는 실행은 매번 정지됩니다.

<h2 id="manage-scheduled-tasks">
  예약된 작업 관리
</h2>

**Code** 탭에서 **Routines** 목록의 작업을 클릭하여 세부 정보 페이지를 엽니다. 여기에서 다음을 수행할 수 있습니다:
...
* [Routines](/docs/ko/routines): 클라우드에서 일정에 따라, API 호출을 통해 또는 GitHub 이벤트에 대응하여 작업을 실행합니다. 컴퓨터가 꺼져 있어도 실행됩니다.

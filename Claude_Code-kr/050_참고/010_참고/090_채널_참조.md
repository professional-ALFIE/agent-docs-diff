...
<Steps>
  ...
  <Step title="테스트">
    ...
    * **`curl`은 성공하지만 Claude에 도달하지 않음**: 세션에서 `/mcp`를 실행하여 서버의 상태를 확인합니다. `failed` 상태는 일반적으로 서버 파일의 종속성 또는 가져오기 오류를 의미합니다. `~/.claude/debug/<session-id>.txt`의 디버그 로그에서 stderr 추적을 확인합니다.

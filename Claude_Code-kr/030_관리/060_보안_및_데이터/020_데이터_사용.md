> 원본: https://code.claude.com/docs/ko/data-usage.md
...
`/feedback` 명령을 사용하여 Claude Code에 대한 피드백을 보내기로 선택한 경우, 피드백을 사용하여 제품 및 서비스를 개선할 수 있습니다. `/feedback`, `/bug`, `/share`를 통해 공유된 대화 기록은 동일한 경로를 통해 보고되며 5년 동안 보관됩니다.

[Claude가 작성한 피드백](/docs/ko/tools-reference#sendfeedback-tool-behavior)을 사용하면 Claude가 피드백 보고서를 작성하고 검토할 수 있도록 머신에 대기열에 넣을 수 있습니다. Claude Code는 초안을 보내기로 선택할 때까지 아무것도 전송하지 않으며, 전송된 초안은 다른 `/feedback` 보고서와 동일한 제출 경로 및 보관 기간을 거칩니다.
...
* 로컬 캐싱: Claude Code 클라이언트는 세션 재개를 활성화하기 위해 `~/.claude/projects/` 아래의 일반 텍스트로 세션 기록을 로컬에 저장합니다(기본값 30일). `cleanupPeriodDays`로 기간을 조정합니다. 저장되는 내용 및 삭제 방법은 [application data](/docs/ko/claude-directory#application-data)를 참조하세요.
  ...
  Claude Desktop 또는 Cowork에서 시작되거나 가장 최근에 계속된 세션의 기록은 [기본적으로 해당 제한에서 제외됩니다](/docs/ko/claude-directory#cleaned-up-automatically).
...
| Microsoft Foundry            | 배포의 [호스팅 옵션](https://platform.claude.com/docs/en/build-with-claude/claude-in-microsoft-foundry#hosting-options)에 따라 달라집니다. Azure에서 호스팅되는 배포의 경우, 프롬프트 및 완성은 Azure 내에 유지되며, 사용 메타데이터 및 Anthropic의 안전 시스템에 의해 플래그된 콘텐츠만 Anthropic으로 나갑니다. Anthropic에서 호스팅되는 배포의 경우, 요청은 AES-256 디스크 암호화를 사용하는 Anthropic 인프라로 라우팅됩니다. |
...
[Claude Code on the web](/docs/ko/claude-code-on-the-web)을 사용할 때, 세션은 기본적으로 로컬이 아닌 Anthropic 관리 가상 머신에서 실행됩니다. 조직이 [자체 호스팅 환경](/docs/ko/self-hosted-environments)으로 라우팅하는 세션은 사용자가 제어하는 인프라에서 실행됩니다. 머신에 유지되는 것과 여전히 Anthropic으로 이동하는 것에 대해서는 [인프라에 유지되는 것](/docs/ko/self-hosted-environments#what-stays-on-your-infrastructure)을 참조하세요. Anthropic 호스팅 클라우드 세션에서:
...
Claude Code는 두 가지 종류의 운영 원격 측정을 전송합니다: 사용 메트릭과 오류 보고서입니다. 아래의 환경 변수를 사용하여 각각을 개별적으로 끌 수 있으며, `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`을 설정하여 모든 필수가 아닌 트래픽을 한 번에 비활성화할 수 있습니다. `DISABLE_TELEMETRY` 또는 `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`을 설정하면 [Remote Control](/docs/ko/remote-control#requirements)이 의존하는 기능 플래그 평가도 비활성화됩니다. `DISABLE_ERROR_REPORTING`은 그렇지 않습니다.
...
`/feedback` 명령을 실행하면 코드를 포함한 대화 기록의 복사본이 Anthropic으로 전송됩니다. `/bug` 및 `/share` 명령은 동일한 경로를 통해 제출됩니다. 피드백 대화 상자에서 보고서를 제출하기 전에 포함할 기록의 양을 선택합니다: 기본값인 현재 세션만 포함하거나, 지난 24시간 또는 7일 동안 같은 프로젝트의 다른 세션도 포함할 수 있습니다. Claude Code는 [Claude가 작성한 피드백](/docs/ko/tools-reference#sendfeedback-tool-behavior)을 동일한 경로를 통해 제출하며, 초안의 검토 화면에서 포함하도록 선택한 경우 기록을 포함합니다. 데이터는 TLS를 통해 전송 중에 암호화되고 Google Cloud Storage에 저장되며, Google Cloud Storage는 기본적으로 저장된 데이터를 암호화합니다. 선택적으로 공개 저장소에 GitHub 이슈가 생성됩니다. 거부하려면 `DISABLE_FEEDBACK_COMMAND` 환경 변수를 `1`로 설정합니다.
...
기본적으로 Amazon Bedrock, Google Cloud의 Agent Platform, Microsoft Foundry 또는 AWS의 Claude Platform을 사용할 때 오류 보고, 원격 측정 및 버그 보고가 비활성화됩니다. 세션 품질 설문조사 및 WebFetch 도메인 안전 검사는 예외이며 제공자와 관계없이 실행됩니다. 서명된 [Claude 앱 게이트웨이](/docs/ko/claude-apps-gateway) 세션에서는 게이트웨이 자격 증명 자체에 의해 Anthropic에 대한 사용 분석, 오류 보고 및 설문조사 평가가 비활성화되며, 이를 다시 활성화할 수 있는 설정이 없습니다. `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`을 설정하여 설문조사를 포함한 모든 필수가 아닌 트래픽을 한 번에 거부할 수 있습니다. 이 변수는 WebFetch 검사 또는 공식 플러그인 마켓플레이스 자동 설치에 영향을 주지 않습니다. 각각 자체 거부 옵션이 있습니다: WebFetch의 경우 [settings](/docs/ko/settings)의 `skipWebFetchPreflight`, 마켓플레이스의 경우 `CLAUDE_CODE_DISABLE_OFFICIAL_MARKETPLACE_AUTOINSTALL`. 다음은 전체 기본 동작입니다:
...
모든 환경 변수는 `settings.json`에 체크인할 수 있습니다([settings reference](/docs/ko/settings-reference) 참조).

호스트 플랫폼이 `CLAUDE_CODE_PROVIDER_MANAGED_BY_HOST`를 설정하면, Google Cloud의 Agent Platform, Amazon Bedrock, Microsoft Foundry 및 AWS의 Claude Platform에서 메트릭은 기본적으로 켜지며 표준 `DISABLE_TELEMETRY` 거부 옵션을 따릅니다. 오류 보고 및 `/feedback` 보고는 해당 제공자에서 기본적으로 꺼진 상태로 유지됩니다.

<h3 id="webfetch-domain-safety-check">
  WebFetch 도메인 안전 검사
</h3>

URL을 가져오기 전에 WebFetch 도구는 요청된 호스트명을 `api.anthropic.com`으로 전송하여 Anthropic에서 유지 관리하는 안전 차단 목록에 대해 확인합니다. 전체 URL, 경로 또는 페이지 내용이 아닌 호스트명만 전송됩니다. Claude Code는 검사를 통과한 호스트명을 5분 동안 캐시하며, 차단되거나 실패한 호스트명은 다음 요청 시 다시 확인합니다.

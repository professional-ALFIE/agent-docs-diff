...
<Steps>
  ...
  <Step title="허용 규칙">
    `allow` 규칙(`allowed_tools` 및 settings.json에서)을 확인합니다. 규칙이 일치하면 도구가 승인됩니다. 도구가 자체적으로 승인하는 호출도 이 단계에서 해결됩니다. 규칙이 필요하지 않습니다: 예를 들어 작업 디렉터리 내의 파일 읽기 또는 [읽기 전용 Bash 명령](/docs/ko/permissions#read-only-commands). [중요 경로](/docs/ko/permission-modes#critical-paths)를 대상으로 하는 `rm` 및 `rmdir` 제거는 허용 규칙으로 절대 승인되지 않습니다: 프롬프트를 표시하는 모드에서는 콜백에 도달하고, Claude Code v2.1.218 이상의 `auto` 모드에서는 [분류기](/docs/ko/permission-modes#eliminate-prompts-with-auto-mode)로 이동하며, `dontAsk` 모드에서는 거부됩니다.
  </Step>
...
| 옵션                                | 효과                                                                                                                                                                    |
| :-------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `allowed_tools=["Read", "Grep"]`  | `Read` 및 `Grep`은 자동으로 승인됩니다. 여기에 나열되지 않은 다른 도구는 여전히 존재하며, 승인이 필요한 호출은 권한 모드 및 `canUseTool`로 통과합니다.                                                                    |
| `disallowed_tools=["Bash"]`       | `Bash` 도구 정의가 요청에서 제거됩니다. Claude는 도구를 보지 못하며 시도할 수 없습니다.                                                                                                              |
| `disallowed_tools=["Bash(rm *)"]` | `Bash`는 계속 사용 가능합니다. `rm *` [작성된 대로](/docs/ko/permissions#bash-rule-limits) 일치하는 호출은 `bypassPermissions`를 포함한 모든 권한 모드에서 거부됩니다. `/bin/rm`을 포함한 다른 `Bash` 호출은 권한 모드로 통과합니다. |
...
나열된 도구는 승인되며, [모든 모드가 자동 승인하지 않는 작업](/docs/ko/permission-modes#actions-no-mode-auto-approves)을 제외하고, 프롬프트를 표시할 다른 모든 호출은 대신 거부됩니다. `default` 모드에서 승인이 필요하지 않은 호출은 나열 여부와 관계없이 실행됩니다. 예를 들어 [읽기 전용 Bash 명령](/docs/ko/permissions#read-only-commands), `Agent`와 같이 실행 전에 묻지 않는 도구, 그리고 작업 디렉토리 내의 파일 읽기가 있습니다. 도구를 Claude의 범위에서 완전히 제거하려면 `disallowedTools`에 단순 이름을 추가합니다.
...
| `dontAsk`           | 프롬프트 대신 거부  | 그렇지 않으면 프롬프트할 모든 호출이 거부됩니다. `allowed_tools` 또는 규칙으로 승인된 호출과 `default` 모드에서 승인이 필요 없는 호출은 실행되며, 커넥터 도구 [조직에서 `ask`로 설정](/docs/ko/mcp#organization-controls-on-connector-tools)하고 사용자 상호작용이 필요한 도구는 사전 승인했더라도 거부됩니다. `rm` 및 `rmdir` 제거가 [중요 경로](/docs/ko/permission-modes#critical-paths)를 대상으로 하는 경우도 거부됩니다. `canUseTool`은 호출되지 않습니다 |
...
권한 모드를 쿼리를 시작할 때 한 번 설정하거나 세션이 활성화된 동안 동적으로 변경할 수 있습니다.
...
모든 권한 프롬프트를 거부로 변환합니다. `canUseTool`을 호출하지 않습니다. `allowed_tools`, `settings.json` 허용 규칙 또는 훅으로 사전 승인된 도구는 정상적으로 실행되며, `default` 모드에서 승인이 필요 없는 호출(예: 작업 디렉토리 내의 파일 읽기 및 `Agent` 호출)도 실행됩니다. 커넥터 도구 [조직에서 `ask`로 설정](/docs/ko/mcp#organization-controls-on-connector-tools), 사용자 상호작용이 필요한 도구, 그리고 [중요 경로](/docs/ko/permission-modes#critical-paths)를 대상으로 하는 `rm` 및 `rmdir` 제거는 허용 규칙이 일치하더라도 거부됩니다. `PreToolUse` 훅 허용도 중요 경로 제거를 해제하지 않습니다.

> 원본: https://code.claude.com/docs/ko/prompt-library.md
...
export const text = {
  ...
  "migrate-a-pattern-across": {
    ...
    teaches: "이전 패턴과 새 패턴을 설명하십시오. Claude에 먼저 모든 위치를 식별하도록 요청하면 호출 사이트가 응답에 나열되므로 놓친 것이 없는지 확인할 수 있습니다. 많은 파일에 걸친 마이그레이션의 경우 [/batch](/docs/ko/commands)를 실행하십시오. Claude가 작업을 승인할 단위로 분할하고 백그라운드 서브에이전트가 변경을 수행하고 단위당 하나의 풀 요청을 엽니다."
  },

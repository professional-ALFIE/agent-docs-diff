> 원본: https://code.claude.com/docs/ko/context-window.md
...
* **아무것도 입력하기 전**: CLAUDE.md, 자동 메모리, MCP 도구 이름, 그리고 스킬 설명이 모두 컨텍스트에 로드됩니다. 사용자의 설정에 따라 [출력 스타일](/docs/ko/output-styles) 또는 [`--append-system-prompt`](/docs/ko/cli-reference)의 텍스트와 같이 추가 항목이 있을 수 있습니다.
...
경로 범위 규칙 및 중첩 CLAUDE.md 파일은 트리거 파일을 읽을 때 메시지 기록에 로드되므로 압축은 다른 모든 것과 함께 이들을 요약합니다. 압축 직후, Claude Code는 세션에서 Claude가 읽거나 편집한 파일 중 최대 5개를 다시 읽으며, 가장 최근에 수정된 파일을 선택하고, 해당 파일에 적용되는 규칙 및 중첩 CLAUDE.md 파일을 다시 로드합니다. 5,000 토큰을 초과하는 파일은 경로 참조로 돌아오며 내용 없이 `Referenced file`로 표시됩니다. 해당 규칙은 여전히 다시 로드됩니다. 규칙이 압축 전체에서 유지되어야 하는 경우 `paths:` 프론트매터를 삭제하거나 프로젝트 루트 CLAUDE.md로 이동하세요.

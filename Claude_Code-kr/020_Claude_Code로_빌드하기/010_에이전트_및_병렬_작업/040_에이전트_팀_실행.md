...
[`CLAUDE_CODE_SUBAGENT_MODEL_FORCE`](/docs/ko/sub-agents#run-every-subagent-on-one-model)는 subagent뿐만 아니라 팀원에게도 적용됩니다.
...
팀을 시작하려면 Claude에게 팀원을 요청합니다. Claude는 에이전트 팀이 활성화되어 있는 동안 [Agent 도구](/docs/ko/tools-reference)를 [`name`](/docs/ko/sub-agents#subagent-names)과 함께 호출할 때 팀원을 시작하며, Claude Code는 확인을 요청하지 않습니다. Claude는 또한 일반 subagent에 자동으로 이름을 지정하여 나중에 메시지를 보낼 수 있으며, 에이전트 팀이 활성화되어 있는 동안 명명된 subagent는 팀원으로 시작되므로, 팀원을 요청하지 않았을 때도 팀이 형성될 수 있습니다.

subagent를 원하면 [에이전트 팀을 끕니다](#claude-spawns-teammates-instead-of-subagents).
...
한 에이전트가 `SendMessage`를 통해 다른 에이전트에게 메시지를 보낼 때, 수신 에이전트는 사용자가 아닌 다른 Claude 세션에서 온 것으로 알려집니다. 팀원은 권한 프롬프트를 승인하거나 사용자를 대신하여 동의를 제공할 수 없으며, 작업이 거부된 팀원은 이를 다른 팀원에게 전달하여 확인을 우회할 수 없습니다. [팀 전체 외부의 다른 Claude Code 세션](/docs/ko/cross-session-messaging#how-a-session-treats-an-incoming-message) 중 하나에서 도착한 메시지에도 동일한 규칙이 적용됩니다.

> 원본: https://code.claude.com/docs/ko/agent-teams.md
...
<Warning>
  에이전트 팀은 실험적이며 기본적으로 비활성화되어 있습니다. [settings.json](/docs/ko/settings)이나 환경에 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`을 설정하여 활성화합니다. 해당 변수가 없으면 세션 시작 시 팀이 설정되지 않고, 팀 디렉토리가 작성되지 않으며, Claude가 팀원을 생성하거나 제안하지 않습니다. 에이전트 팀은 세션 재개, 작업 조율, 종료 동작 관련 [알려진 제한 사항](#limitations)이 있습니다.
</Warning>

에이전트 팀을 사용하면 함께 작동하는 여러 Claude Code 인스턴스를 조율할 수 있습니다. 한 세션이 팀 리더 역할을 하여 작업을 조율하고, 작업을 할당하며, 결과를 종합합니다. 팀원들은 독립적으로 작동하며, 각각 자신의 컨텍스트 윈도우에서 작동하고, 서로 직접 통신합니다. 리더를 거치지 않고 개별 팀원과 직접 상호작용할 수도 있습니다.

팀을 설정하기 전에 더 가벼운 옵션이 작업을 수행하는지 확인합니다. [Subagents](/docs/ko/sub-agents)는 단일 세션 내에서 작동하며, [cross-session messaging](/docs/ko/cross-session-messaging)을 통해 Claude는 직접 실행하는 세션 간에 결과를 전달할 수 있습니다.
...
에이전트 팀과 [subagents](/docs/ko/sub-agents) 모두 작업을 병렬화할 수 있지만, 다르게 작동합니다. 별도의 세션에서 팀 없이 서로에게 메시지를 전달하는 경우 [교차 세션 메시징](/docs/ko/cross-session-messaging)을 참조합니다.

<Frame caption="Subagents는 결과를 메인 에이전트에게 보고합니다. 에이전트 팀에서는 팀원들이 작업 목록을 공유하고, 작업을 요청하며, 서로 직접 통신합니다.">
...
| **통신**    | 호출자에게 결과를 반환합니다. Claude가 생성할 때 이름을 지정한 Subagents는 [서로 메시지를 전달](/docs/ko/sub-agents#what-loads-at-startup)할 수도 있습니다 | 팀원들이 서로 직접 메시지 전송                                                                          |
| **조율**    | 메인 에이전트가 모든 작업 관리                                                                                             | [Task 도구가 있는 에이전트](/docs/ko/tools-reference#task-tool-availability)의 경우 메시지와 공유 작업 목록을 통한 자체 조율 |
...
에이전트 팀을 활성화하면 일반적인 위임도 변경됩니다. Claude는 [자체적으로 서브에이전트의 이름을 지정](/docs/ko/sub-agents#subagent-names)할 수 있으며, 에이전트 팀이 활성화된 동안 Claude가 이름을 지정한 서브에이전트는 팀원으로 시작되므로 사용자가 요청하지 않았을 때도 팀이 형성될 수 있습니다. 자세한 내용은 [Claude가 에이전트 팀을 시작하는 방법](#how-claude-starts-agent-teams)을 참조하십시오. 이 동작을 끄려면 [Claude가 서브에이전트 대신 팀원을 생성합니다](#claude-spawns-teammates-instead-of-subagents)를 참조하십시오.

팀원 생성에는 대화형 세션도 필요합니다. `-p` 플래그를 포함한 [비대화형 모드](/docs/ko/headless)에서 Agent SDK 세션을 포함하여 Claude는 팀원을 생성하지 않으며, Claude가 이름을 지정한 서브에이전트는 에이전트 팀이 활성화되어 있어도 일반적인 [서브에이전트](/docs/ko/sub-agents)로 실행됩니다.
...
그러면 Claude는 [공유 작업 목록](/docs/ko/interactive-mode#task-list)을 채우고, [Task 도구가 있는 세션](/docs/ko/tools-reference#task-tool-availability)에서 각 관점에 대한 팀원들을 생성하며, 문제를 탐색하고, 완료되었을 때 발견을 종합합니다.

Claude는 때때로 팀을 만드는 대신 [서브에이전트](/docs/ko/sub-agents)를 사용할 수 있습니다. 서브에이전트는 팀원과 동일한 에이전트 패널에 나타나므로, 패널만으로는 팀이 형성되었는지 확인할 수 없습니다. Claude가 대신 서브에이전트를 생성한 경우, 다시 요청하고 명시적으로 에이전트 팀을 요청합니다.

리더의 터미널은 프롬프트 입력 아래의 에이전트 패널에 팀원들을 나열합니다. 패널에서:

* **위아래 화살표**: 팀원 선택
* **Enter**: 선택한 팀원의 대화 기록을 열고 직접 메시지 전송
* **Escape**: 선택 해제. 팀원의 대화 기록을 보고 있는 동안 Escape는 해당 팀원의 현재 턴을 중단합니다.
...
기본값을 재정의하려면 `~/.claude/settings.json`에서 [`teammateMode`](/docs/ko/settings-reference#teammatemode)를 설정합니다:
...
`--teammate-mode` 플래그는 실험적이며 `claude --help`에 나타나지 않습니다.
...
Claude Code는 다음 중 적용되는 첫 번째 항목에서 각 팀원의 모델을 선택합니다:

1. 스폰 프롬프트가 해당 팀원을 위해 지정하는 모델입니다.
2. [subagent 정의](#use-subagent-definitions-for-teammates)에서 생성된 팀원의 경우, 정의의 `model`이며, 여기서 `inherit`는 리더의 모델을 선택합니다.
3. [`CLAUDE_CODE_SUBAGENT_MODEL`](/docs/ko/model-config#environment-variables)이 `inherit` 이외의 값으로 설정된 경우입니다.
4. 리더의 현재 모델입니다.

[`CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1`](/docs/ko/sub-agents#run-every-subagent-on-one-model)을 설정하면, 처음 두 소스는 적용되지 않습니다. Claude Code는 `CLAUDE_CODE_SUBAGENT_MODEL`이 `inherit` 이외의 값으로 설정된 경우 모든 팀원의 모델을 선택하고, 그렇지 않으면 리더의 현재 모델에서 선택합니다. Claude Code v2.1.257 이상이 필요합니다.

v2.1.251 이전에는 `CLAUDE_CODE_SUBAGENT_MODEL`이 이 순서에서 첫 번째였습니다.

<Note>
  `teammateDefaultModel`은 v2.1.234에서 제거되었습니다. Claude Code는 남은 값을 무시합니다. 대신 프롬프트에서 모델을 지정합니다.
</Note>

Claude Code는 팀원을 위해 선택한 모델을 조직의 [`availableModels`](/docs/ko/model-config#restrict-model-selection) 허용 목록에 대해 확인합니다. 허용 목록이 값을 차단할 때, Claude Code는 다른 모델을 대체합니다:

* **`opus`와 같은 패밀리 별칭**: Anthropic API 및 AWS의 Claude Platform에서 Claude Code는 팀원을 허용 목록이 허용하는 해당 패밀리의 최신 버전에서 실행합니다. 제공자별 모델 ID가 있는 제공자에서 [대체가 작동하지 않는](/docs/ko/model-config#restrict-model-selection) 경우, 차단된 별칭은 다음 글머리 기호에 따라 다른 차단된 값처럼 폴백합니다.
* **차단된 다른 값(제공자별 대체가 작동하지 않는 제공자의 패밀리 별칭 포함, 또는 패밀리에 허용된 버전이 없는 경우)**: Claude Code는 팀원을 리더의 모델에서 실행합니다. `CLAUDE_CODE_SUBAGENT_MODEL`을 설정한 경우, Claude Code는 이러한 동일한 규칙에 따라 먼저 해당 모델을 시도합니다.
...
<h3 id="have-teammates-plan-before-implementing">
  팀원이 구현하기 전에 계획하도록 하기
</h3>

복잡하거나 위험한 작업의 경우, 팀원들이 구현하기 전에 계획하도록 할 수 있습니다. 리더가 [plan mode](/docs/ko/permission-modes#analyze-before-you-edit-with-plan-mode)에 있는 동안 Claude가 생성하는 팀원은 계획이 준비될 때까지 읽기 전용 plan mode에서 작동합니다. 먼저 리더를 plan mode로 전환한 후 팀원을 요청합니다:

```text wrap theme={null}
Spawn an architect teammate to refactor the authentication module.
```

팀원이 계획을 마치면, 리더에게 계획 승인 요청을 보냅니다. Claude Code는 리더가 검토하지 않고도 요청이 도착하는 즉시 리더의 세션에서 계획을 승인합니다. 팀원의 편집 및 명령은 여전히 [Permissions](#permissions)에 설명된 권한 프롬프트를 거칩니다. 승인되면, 팀원은 plan mode를 종료하고 구현을 시작합니다.
...
[Task 도구가 없는](/docs/ko/tools-reference#task-tool-availability) 에이전트는 공유 작업 목록 대신 메시지를 통해 조율합니다.
...
팀을 시작하려면 Claude에게 팀원을 요청합니다. Claude는 에이전트 팀이 활성화되어 있는 동안 [Agent 도구](/docs/ko/tools-reference)를 [`name`](/docs/ko/sub-agents#subagent-names)과 함께 호출할 때 팀원을 시작하며, 호출이 [fork](/docs/ko/sub-agents#fork-the-current-conversation)이거나 호출 자체에서 `isolation`을 전달하지 않는 한입니다. Claude Code는 확인을 요청하지 않습니다.

Claude는 또한 일반 subagent에 자동으로 이름을 지정하여 나중에 메시지를 보낼 수 있습니다. 이러한 호출은 동일한 규칙을 따르므로, 팀원을 요청하지 않았을 때도 팀이 형성될 수 있습니다. subagent를 원하면 [에이전트 팀을 끕니다](#claude-spawns-teammates-instead-of-subagents).
...
Claude Code는 메시지가 일반 텍스트이든 계획 승인이나 종료 요청과 같은 구조화된 프로토콜 메시지이든 수신자의 메일박스 파일에 대한 쓰기가 성공할 때만 메시지를 전송된 것으로 보고합니다. 쓰기가 실패하면(예: 디스크가 가득 찼거나 메일박스 디렉토리를 쓸 수 없는 경우), 전송 에이전트는 오류를 받고 아무것도 전송되지 않습니다. [팀원의 받은편지함에 쓰기 실패](/docs/ko/errors#failed-to-write-to-a-teammate-inbox)에서 오류 메시지 및 복구 단계를 참조합니다.

Claude Code는 작업 종속성을 자동으로 관리합니다: 팀원이 다른 작업이 종속된 작업을 완료하면, 차단된 작업은 사용자의 조치 없이 차단 해제됩니다.
...
Claude Code는 세션 시작 시 이 둘을 자동으로 생성하고 팀원들이 참여하거나, 유휴 상태가 되거나, 떠날 때 업데이트합니다. 팀 구성 디렉토리는 세션이 끝날 때 제거됩니다. 작업 목록 디렉토리는 로컬에 유지되며 절대 업로드되지 않으므로, 재개된 세션은 작업을 유지합니다. 보존은 세션 트랜스크립트에 대해 이미 제어하는 것과 동일한 [`cleanupPeriodDays`](/docs/ko/settings-reference#cleanupperioddays)에 의해 관리되며, [보존 정리 규칙](/docs/ko/claude-directory#cleaned-up-automatically)을 따릅니다.
...
팀 구성에는 각 팀원의 이름과 에이전트 ID가 있는 `members` 배열이 포함됩니다. 리더의 항목은 항상 에이전트 유형 `team-lead`를 전달합니다. 팀원의 항목은 리더가 팀원을 생성할 때 명명한 에이전트 유형(예: [내장 유형](/docs/ko/sub-agents#built-in-subagents) 또는 [subagent 정의](#use-subagent-definitions-for-teammates))을 전달하며, 리더가 아무것도 명명하지 않으면 필드를 생략합니다. 팀원들은 이 파일을 읽어 다른 팀 멤버를 발견할 수 있습니다.
...
두 표시 모드에서 팀원을 생성할 때, 프로젝트, 사용자, 또는 관리형 [subagent 범위](/docs/ko/sub-agents#choose-the-subagent-scope)의 [subagent](/docs/ko/sub-agents) 유형을 참조할 수 있습니다. 이를 통해 보안 검토자 또는 테스트 실행자와 같은 역할을 한 번 정의하고 위임된 subagent와 에이전트 팀 팀원 모두로 재사용할 수 있습니다.
...
Claude Code는 명명한 subagent 정의를 읽고 다음 부분을 팀원에게 적용합니다. 부분이 팀원의 [표시 모드](#choose-a-display-mode)에 따라 달라지는 경우, 항목에서 그렇게 표시합니다:

* **`tools`**: Claude Code는 팀원을 정의의 `tools` 목록의 도구로 제한합니다. 프로세스 내 팀원의 경우, Claude Code는 해당 목록에 `SendMessage`를 추가하며, [Task 도구가 있는 세션](/docs/ko/tools-reference#task-tool-availability)에서는 `TaskCreate`, `TaskGet`, `TaskList`, `TaskUpdate`도 추가합니다.
* **`model`**: Claude Code는 생성 프롬프트가 모델을 명명하지 않을 때 두 표시 모드에서 정의의 `model`을 사용합니다. [Claude Code가 팀원의 모델을 선택하는 방법](#specify-teammates-and-models)을 참조합니다.
* **본문**: 프로세스 내 팀원의 경우, Claude Code는 정의의 본문을 기본 시스템 프롬프트에 추가 지시로 추가합니다. 분할 창 팀원의 경우, Claude Code는 본문을 기본 시스템 프롬프트 대신 사용합니다.
* **`skills`**: Claude Code는 두 표시 모드에서 팀원에게 정의의 `skills`을 적용하지 않습니다. 팀원은 프로젝트 및 사용자 설정에서 skills을 로드합니다.
* **`mcpServers`**: 분할 창 팀원의 경우, Claude Code는 [해당 필드의 규칙](/docs/ko/sub-agents#scope-mcp-servers-to-a-subagent)에 따라 정의의 `mcpServers`를 적용하며, 이는 `--agent`로 시작된 세션도 포함합니다. 프로세스 내 팀원은 필드를 무시하고 프로젝트 및 사용자 설정에서 MCP 서버를 로드합니다.

<h3 id="permissions">
  권한
</h3>

팀원들은 리더의 권한 모드로 시작하며, [`dontAsk` 모드](/docs/ko/permission-modes#allow-only-pre-approved-tools-with-dontask-mode) 제외하고, 이는 상속하지 않습니다. 리더가 `--dangerously-skip-permissions`로 실행되면, 모든 팀원도 그렇게 합니다. 생성 후, 개별 팀원의 권한 모드를 변경할 수 있지만, 생성 시 팀원별 권한 모드를 설정할 수 없습니다.

팀원 권한 프롬프트는 리더 세션에 나타나므로, 거기서 직접 승인합니다. [계획 승인](#have-teammates-plan-before-implementing)은 설계된 예외입니다: 리더 세션은 사용자에게 별도의 프롬프트 없이 팀원 계획 승인을 부여합니다.

<h4 id="messages-between-agents">
  에이전트 간 메시지
</h4>

한 에이전트가 `SendMessage`를 통해 다른 에이전트에게 메시지를 보낼 때, Claude Code는 수신 에이전트에게 사용자가 아닌 다른 Claude 세션에서 온 것으로 알립니다. 팀원은 권한 프롬프트를 승인하거나 사용자를 대신하여 동의를 제공할 수 없으며, 작업이 거부된 팀원은 이를 다른 팀원에게 전달하여 확인을 우회할 수 없습니다. [팀 전체 외부의 다른 Claude Code 세션](/docs/ko/cross-session-messaging#how-a-session-treats-an-incoming-message) 중 하나에서 도착한 메시지에도 동일한 규칙이 적용됩니다.

[자동 모드](/docs/ko/permission-modes#eliminate-prompts-with-auto-mode)에서, 분류기는 에이전트 간 메시지에 두 가지 확인을 적용합니다:

* 다른 에이전트에서 전달된 승인 주장을 사용자로부터의 확인이 아닌 신뢰할 수 없는 입력으로 취급합니다.
* 종료 요청이나 계획 승인 응답과 같은 구조화된 프로토콜 메시지이든 일반 메시지이든 Claude Code가 전달하기 전에 각 메시지를 검토합니다. 차단된 메시지는 수신자에게 도달하지 않습니다.

<h3 id="context-and-communication">
  컨텍스트 및 통신
</h3>

각 팀원은 자신의 컨텍스트 윈도우를 가집니다. 생성될 때, 팀원은 일반 세션과 동일한 프로젝트 컨텍스트를 로드합니다: CLAUDE.md, MCP 서버, skills. 또한 리더의 생성 프롬프트를 받습니다. 리더의 대화 기록은 전달되지 않습니다.

**팀원들이 정보를 공유하는 방법:**

* **자동 메시지 전달**: 팀원들이 메시지를 보낼 때, 자동으로 수신자에게 전달됩니다. 리더가 업데이트를 폴링할 필요가 없습니다.
* **유휴 알림**: 팀원이 완료되고 중지되면, 자동으로 리더에게 알립니다. 최종 답변을 알림에 포함합니다. API 오류에서 턴이 끝난 팀원은 실패했음을 리더에게 알리고 오류 텍스트를 포함합니다.
* **공유 작업 목록**: [Task 도구가 있는 에이전트](/docs/ko/tools-reference#task-tool-availability)는 작업 상태를 보고 사용 가능한 작업을 요청할 수 있습니다.
...
프로세스 내 팀원의 요청은 메인 대화의 [캐시 TTL 버킷](/docs/ko/prompt-caching#which-ttl-each-request-gets) 외부에 있으므로, 캐시는 Claude 구독에서도 기본적으로 5분 동안 유지됩니다. 1시간 동안 유지하려면, [`subagentPromptCacheTtl`](/docs/ko/settings-reference#subagentpromptcachettl)을 `1h`로 설정합니다. API는 1시간 캐시 쓰기를 더 높은 요금으로 청구합니다.
...
대부분의 워크플로우에 대해 3-5명의 팀원으로 시작합니다. 이는 병렬 작업과 관리 가능한 조율의 균형을 맞춥니다. 15개의 독립적인 작업이 있으면, 3명의 팀원이 좋은 시작점입니다.
...
<h3 id="claude-spawns-teammates-instead-of-subagents">
  Claude가 서브에이전트 대신 팀원을 생성함
</h3>

에이전트 팀이 활성화되어 있는 동안, Claude가 리더의 세션에서 이름을 지정한 서브에이전트는 팀원으로 시작됩니다. Claude는 [자체적으로 서브에이전트의 이름을 지정할 수 있으므로](#how-claude-starts-agent-teams), 이는 팀 작업으로 프레임하지 않은 위임 중에 발생할 수 있습니다.

명명된 서브에이전트가 다시 서브에이전트로 시작되도록 하려면, `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`를 `0`으로 설정하여 에이전트 팀을 끕니다:
...
새 세션을 시작할 필요가 없습니다: Claude Code는 저장할 때 설정 파일 `env` 값을 실행 중인 세션에 다시 적용하며, Claude가 서브에이전트를 생성할 때마다 변수를 다시 읽으므로, Claude가 명명한 다음 서브에이전트는 서브에이전트로 시작됩니다.

사용자 `settings.json`에서 변수를 `0`으로 설정하면 셸 내보내기를 재정의합니다. 다른 설정 소스는 여전히 에이전트 팀을 활성화할 수 있습니다:

* **더 높은 우선순위 설정 파일**: 프로젝트 설정, 로컬 설정 및 `--settings` 페이로드는 사용자 설정 후에 적용되므로, 이들 중 하나에서 변수를 `1`로 설정하는 `env` 항목이 우선합니다. [설정 우선순위](/docs/ko/settings#settings-precedence)를 참조합니다.
* **관리되는 설정**: [관리되는 설정](/docs/ko/server-managed-settings)은 다른 모든 소스 후에 적용됩니다. 조직에서 에이전트 팀을 활성화한 경우, 관리자에게 관리되는 값을 변경하도록 요청합니다.

변경 후, Claude는 여전히 서브에이전트의 이름을 지정할 수 있으며, 이름은 [`SendMessage` 주소](/docs/ko/sub-agents#resume-subagents)로 계속 작동합니다. Claude는 각 서브에이전트가 완료될 때 그 결과를 받습니다.
...
리더 또는 다른 팀원의 메시지는 실패한 API 요청을 재시도하기 위해 대기 중인 in-process 팀원을 깨우므로, 전체 재시도 지연을 기다리지 않고 즉시 재시도합니다.

리더는 모든 작업이 실제로 완료되기 전에 팀이 완료되었다고 결정할 수 있습니다. 이 경우 계속하도록 말합니다.
...
* **In-process 팀원의 백그라운드 서브에이전트 없음**: in-process 팀원의 자체 서브에이전트는 포그라운드에서 실행됩니다. 팀원의 백그라운드 작업은 리더의 프로세스보다 오래 지속될 수 없기 때문입니다. Claude Code는 팀원이 `background: true`를 설정하는 서브에이전트 정의를 생성할 때 오류를 반환합니다. 팀원의 `run_in_background: true` 요청도 실패하며, [Claude Code가 포그라운드 또는 백그라운드를 선택하는 방법](/docs/ko/sub-agents#run-subagents-in-foreground-or-background)에 설명된 대로 오류가 발생하거나 포그라운드에서 자동으로 실행됩니다. 주 대화에서 시작된 서브에이전트는 [백그라운드 기본값](/docs/ko/sub-agents#run-subagents-in-foreground-or-background)을 따릅니다.
* **리더가 고정됨**: 주 세션은 수명 동안 리더입니다. 팀원을 리더로 승격하거나 리더십을 이전할 수 없습니다.
* **생성 시 권한 설정**: 팀원들은 [권한](#permissions) 아래에 설명된 권한 모드로 시작합니다. 생성 후 개별 팀원의 권한 모드를 변경할 수 있지만, 생성 시 팀원별 권한 모드를 설정할 수 없습니다.
...
* **세션 간 메시징**: [cross-session messaging](/docs/ko/cross-session-messaging)을 사용하면 Claude가 직접 실행하는 세션 간에 발견 사항을 전달할 수 있습니다

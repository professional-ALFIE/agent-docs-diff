...
* 터미널에서 직접 시작하는 세션으로, 호출한 방식대로 실행됩니다. 이러한 세션을 포함하려면 `PATH`의 이전 디렉터리에 `claude`라는 이름의 스크립트를 배치하여 실제 바이너리로 런처를 실행하십시오. 관리되는 심볼릭 링크를 교체하지 마십시오. 자체 생성은 `PATH`를 참조하지 않으므로 두 런처는 절대 스택되지 않습니다.

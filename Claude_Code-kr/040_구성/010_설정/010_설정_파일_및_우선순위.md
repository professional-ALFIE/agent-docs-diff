...
* **Claude Code는 저장소 파일의 키를 무시합니다.** [설정 인덱스](/docs/ko/settings-reference#settings-index)의 범위 열에서 `User, local, or managed`, `User or managed`, `Managed`, 또는 `Global config`를 찾습니다. 이러한 키는 [`autoContinueAtUsageLimit`](/docs/ko/settings-reference#autocontinueatusagelimit)를 제외하고 공유 파일에서 적용되지 않습니다. 저장소 파일은 여전히 키를 끌 수 있습니다. 파일이 키를 설정하고 사용자, `--settings`, 또는 관리되는 값이 없으면, Claude Code는 설정을 끈 것으로 읽습니다. `Global config` 키는 `~/.claude.json`에서만 적용됩니다.

...
Claude Code는 `ctrl+x` 접두사에서 다음과 같은 기본 코드를 바인딩합니다: `Chat`의 `ctrl+x ctrl+k`, `ctrl+x ctrl+e`, `ctrl+x enter`, `ctrl+x ctrl+a`, 그리고 `ctrl+x tab`, `Task`의 `ctrl+x ctrl+b`, 그리고 `DiffPanel`의 `ctrl+x b`. `ctrl+x enter` 코드는 v2.1.247 이상이 필요하며, `ctrl+x b`, `ctrl+x ctrl+a`, 그리고 `ctrl+x tab`은 v2.1.260 이상이 필요합니다. `ctrl+x` 자체를 단일 키 바인딩으로 되찾으려면 모두 바인딩 해제합니다:

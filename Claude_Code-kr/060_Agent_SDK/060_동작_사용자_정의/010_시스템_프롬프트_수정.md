...
기본적으로, Claude Code는 세션의 첫 번째 요청에서 시스템 프롬프트를 한 번 빌드하며, `append` 텍스트 또는 사용자 정의 프롬프트가 포함되고, 세션에 기록됩니다. 세션이 압축될 때까지, 이후의 모든 요청은 `resume` 또는 `continue`로 세션으로 돌아간 후에도 해당 기록된 프롬프트를 사용합니다. 나중의 호출에서 다른 `append` 또는 사용자 정의 프롬프트를 전달하면, 세션이 압축되거나 새 세션에서 적용됩니다.

`--bare`를 `extraArgs`를 통해 전달하거나 `CLAUDE_CODE_SIMPLE=1`을 설정하여 Claude Code를 [bare mode](/docs/ko/headless#start-faster-with-bare-mode)에서 시작하면, `systemPrompt`의 객체 형식에서 `snapshot: true`를 설정하지 않는 한 기록이 꺼집니다. `append` 또는 사용자 정의 프롬프트를 기본적으로 기록하려면 Claude Code v2.1.265 이상이 필요하며, TypeScript Agent SDK는 v0.3.265부터 이를 번들로 제공합니다. Claude Code v2.1.268 이전에는 [feature flags를 가져오지 않는](/docs/ko/env-vars#features-that-need-feature-flag-fetching) 세션(Amazon Bedrock, Google Cloud의 Agent Platform, Microsoft Foundry의 세션 포함)이 모든 요청에서 프롬프트를 다시 빌드했으며 `snapshot`은 효과가 없었습니다.

대신 모든 요청에서 프롬프트를 다시 빌드하려면, TypeScript SDK의 `systemPrompt`의 객체 형식에서 `snapshot: false`를 설정하십시오: `{ type: "preset", preset: "claude_code", append, snapshot: false }` 또는 `{ type: "custom", prompt, snapshot: false }`. 프롬프트 표현을 반복할 때 또는 애플리케이션이 동일한 세션을 재개하는 호출 사이에 `append`를 변경할 때 이 형식을 사용하십시오. `snapshot` 필드는 `@anthropic-ai/claude-agent-sdk` v0.3.257 이상이 필요합니다.

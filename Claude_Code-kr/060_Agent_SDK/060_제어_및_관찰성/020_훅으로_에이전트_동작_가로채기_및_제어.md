> 원본: https://code.claude.com/docs/ko/agent-sdk/hooks.md
...
콜백이 타임아웃을 초과하면 Claude Code는 이를 취소하고 실패한 훅으로 처리합니다: 콜백의 출력을 버리고 세션은 중단되지 않고 계속됩니다. 다음에 일어나는 일은 이벤트에 따라 다릅니다:
...
* `Stop` 및 `SubagentStop`: Claude Code는 경고를 표시하고 에이전트는 정상적으로 중지됩니다.

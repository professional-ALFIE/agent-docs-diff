...
장치 관리 도구를 통해 키를 배포합니다. [서버 관리 설정](/docs/ko/server-managed-settings)은 이미 조직에 인증된 계정에만 도달하므로 개발자의 첫 로그인을 리디렉션할 수 없습니다. 조직에서 서버 관리 설정도 배포하는 경우 두 위치에 키를 설정합니다: 관리형 설정 소스는 [병합되지 않으며](/docs/ko/server-managed-settings#settings-precedence) 캐시된 서버 관리 설정은 장치 관리 파일을 바꾸며, 몇 가지 [키별 예외](/docs/ko/server-managed-settings#per-key-exceptions-across-managed-sources)를 제외합니다. `forceLoginOrgUUID`와 `forceLoginMethod`의 `"claudeai"` 및 `"console"` 값은 이러한 예외 중 하나가 아니므로 두 위치에 유지합니다.

...
| `remote-settings.json` | 조직의 [server-managed settings](/docs/ko/server-managed-settings)의 캐시된 복사본이거나, 조직이 설정한 경우 `{}`입니다. 세션이 [fetches them](/docs/ko/server-managed-settings#platform-availability)할 때만 존재합니다. Claude Code는 시작 시 및 세션 중 매시간 업데이트를 확인합니다. 로그아웃할 때 삭제됩니다. |

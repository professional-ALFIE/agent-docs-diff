> 원본: https://code.claude.com/docs/ko/computer-use.md
...
화면 제어는 다른 것이 도달할 수 없는 것들을 위해 예약되어 있습니다. 네이티브 앱, iOS Simulator와 같은 시뮬레이터, API가 없는 도구입니다.
...
Finder를 승인하면 Claude가 데스크톱, Dock 또는 Finder 윈도우를 클릭할 수 있습니다.
...
한 번에 하나의 세션만 컴퓨터를 사용할 수 있습니다. 세션은 첫 번째 컴퓨터 사용 작업에서 잠금을 획득하고 작업이 완료될 때가 아니라 세션이 종료될 때 해제합니다. 두 번째 세션의 컴퓨터 사용은 잠금을 유지하는 세션의 이름을 지정하는 오류로 실패합니다. 먼저 해당 세션을 종료합니다.
...
Claude가 각 턴에서 처음으로 컴퓨터를 사용할 때 macOS 알림이 나타납니다: "Claude가 컴퓨터를 사용 중입니다 · Esc를 눌러 중지하세요." 어디서나 `Esc`를 눌러 현재 작업을 즉시 중단하거나 터미널에서 `Ctrl+C`를 누릅니다. 어느 쪽이든 Claude는 중지하고, 앱을 표시하고, 제어를 사용자에게 반환합니다. 세션은 종료될 때까지 [컴퓨터 사용 잠금](#one-session-at-a-time)을 유지합니다.
...
Claude는 마우스를 사용하는 것처럼 시뮬레이터를 제어합니다. 이 흐름은 CLI에 적용됩니다. Desktop 앱에서는 동일한 요청이 화면 제어 대신 [iOS Simulator 창](/docs/ko/desktop-ios-simulator)을 엽니다.

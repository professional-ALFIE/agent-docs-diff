> 원본: https://code.claude.com/docs/ko/glossary.md
...
`--bare`를 사용하면 Claude Code는 `--add-dir`으로 전달하는 디렉터리의 스킬을 제외하고 훅, 스킬, 사용자 정의 명령, 서브에이전트, 플러그인, MCP 서버, auto memory 또는 CLAUDE.md를 로드하지 않고 시작합니다. CI 및 스크립트된 호출에 권장되며, 모든 머신에서 동일한 결과가 필요한 경우에 사용합니다.
...
Claude의 시스템 프롬프트를 수정하여 응답 동작, 톤 또는 형식을 변경하는 구성입니다. [CLAUDE.md](#claude-md)와 달리 Claude Code가 시스템 프롬프트 이후 사용자 메시지로 전달하는 것과 달리, 출력 스타일은 시스템 프롬프트 자체를 변경합니다.

...
Claude Code는 스크립트를 다시 실행하고 이 메시지를 표시하기 전에 요청을 최대 2회 더 재시도하므로, 실패는 3번의 시도 내에 나타납니다. v2.1.208 이전에는 Claude Code가 전체 [재시도 예산](#automatic-retries)을 자리 표시자 자격증명으로 요청을 재전송하는 데 사용한 후 일반적인 `401` 인증 오류 대신 스크립트 실패를 보고했습니다.
...
`expected`는 서버의 OAuth 메타데이터의 발급자이고, `received`는 리디렉션이 전달한 `iss` 값입니다. `iss` 매개변수가 없는 로그인은 서버의 메타데이터가 `authorization_response_iss_parameter_supported`를 설정하지 않으면 확인을 통과합니다. 이 경우 Claude Code는 로그인을 실패합니다.
...
[`claude import`](/docs/ko/cli-reference#cli-commands)를 실행했으며 Claude Code가 가져오기 흐름이 꺼져 있음을 발견했으므로 명령어가 이 메시지를 인쇄하는 대신 종료 코드 1로 종료됩니다. v2.1.222 이전에는 가져오기 흐름이 꺼진 빌드가 `import`를 프롬프트로 처리하고 이 메시지를 인쇄하는 대신 대화형 세션을 시작했습니다.
...
* 브랜치를 이름 지정하지 않으려면 `git fetch origin`을 실행한 후 `git remote set-head origin --auto`를 실행하세요. 이는 원격에 기본 브랜치가 무엇인지 묻습니다. 원격이 기본 브랜치를 광고하지 않을 때 실패합니다. 비어 있거나 HEAD가 아무도 푸시하지 않은 브랜치를 가리키기 때문입니다. 대신 브랜치를 명시적으로 이름 지정하세요. 클론이 해당 브랜치를 가져오지 않을 때 `error: Not a valid ref`로 실패합니다. 먼저 위와 같이 refspec을 넓히세요.
...
* 대화형 사용의 경우 실제 터미널에서 `claude`를 실행하세요: ISE가 아닌 Windows Terminal 또는 PowerShell 콘솔, IDE의 통합 터미널이 아닌 출력 창.

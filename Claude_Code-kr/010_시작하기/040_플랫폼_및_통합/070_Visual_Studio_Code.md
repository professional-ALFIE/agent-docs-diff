...
<Steps>
  <Step title="Claude Code 패널 열기">
    ...
    * **활동 표시줄**: 왼쪽 사이드바의 Spark 아이콘을 클릭하여 세션 목록을 엽니다. 세션을 클릭하여 전체 편집기 탭으로 열거나 새로운 세션을 시작할 수 있습니다. 이 아이콘은 항상 활동 표시줄에 표시됩니다.
...
파일을 프롬프트 상자로 드래그할 때 `Shift`를 누르고 있으면 첨부 파일로 추가할 수 있습니다. 첨부 파일의 X를 클릭하여 컨텍스트에서 제거합니다.

<h3 id="resume-past-conversations">
  과거 대화 재개
</h3>

Claude Code 패널 상단의 **Session history** 버튼을 클릭하여 대화 기록에 액세스합니다. 키워드로 검색하거나 시간별로 찾아볼 수 있습니다. 모든 대화를 클릭하여 전체 메시지 기록으로 재개합니다. 세션 재개에 대한 자세한 내용은 [Manage sessions](/docs/ko/sessions)를 참조하세요.

새 세션은 첫 번째 메시지를 기반으로 AI가 생성한 제목을 받습니다. 세션 위에 마우스를 올려 이름 바꾸기 및 보관 작업을 표시합니다. 설명적인 제목을 지정하도록 이름을 바꾸거나 목록 하단의 **Archived sessions** 그룹으로 이동하도록 보관합니다.
...
변경 사항을 적용한 후 배너가 표시되어 Claude Code를 다시 시작하라는 메시지를 표시합니다.
...
| 매개변수      | 설명                                                                                                                                                                                                  |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prompt`  | 프롬프트 상자에 미리 채울 텍스트. URL 인코딩되어야 함. 프롬프트는 미리 채워지지만 자동으로 제출되지 않음.                                                                                                                                      |
| `session` | 새 대화를 시작하는 대신 재개할 세션 ID. 세션은 VS Code에서 현재 열려 있는 작업 영역에 속해야 함. 세션을 찾을 수 없으면 새 대화가 시작됨. 세션이 이미 탭에서 열려 있으면 해당 탭이 포커스됨. 프로그래밍 방식으로 세션 ID를 캡처하려면 [대화 계속하기](/docs/ko/headless#continue-conversations)를 참조하십시오. |
...
VS Code 탭 대신 터미널 세션을 시작하려면 CLI의 `claude-cli://` 핸들러를 사용합니다. [링크에서 세션 시작하기](/docs/ko/deep-links)를 참조하십시오.

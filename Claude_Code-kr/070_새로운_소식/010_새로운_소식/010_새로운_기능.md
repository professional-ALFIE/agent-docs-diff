> 원본: https://code.claude.com/docs/ko/whats-new.md
...
주간 개발자 다이제스트는 업무 방식을 바꿀 가능성이 가장 높은 기능들을 강조합니다. 각 항목에는 실행 가능한 코드, 짧은 데모, 그리고 전체 문서로의 링크가 포함됩니다. 모든 버그 수정 및 사소한 개선 사항은 [changelog](/docs/en/changelog)를 참조하십시오.

<Update label="Week 34" description="2026년 8월 17–21일" tags={["v2.1.234–v2.1.239"]}>
  **`/design`**: Claude Design의 아트보드 워크플로우를 CLI 및 Claude Code Desktop으로 가져오는 연구 미리보기로, 아티팩트를 기반으로 구축되어 Claude가 UI용 편집 가능한 아트보드를 작성하고 선택한 것을 구현합니다.

  이번 주의 다른 기능들: 기본 제공 **Concise output style**은 Claude가 결과로 시작하고 전문을 건너뜁니다. `claude remote-control`을 실행하는 모든 머신은 휴대폰의 **device card**로 표시되므로 Code 탭에서 세션을 시작할 수 있습니다. 그리고 \*\*`ANTHROPIC_DEFAULT_MODEL`\*\*은 새 세션이 시작되는 모델을 설정합니다.

  [Week 34 다이제스트 읽기 →](/docs/ko/whats-new/2026-w34)
</Update>

<Update label="Week 33" description="2026년 8월 10–14일" tags={["v2.1.225–v2.1.233"]}>
  **Desktop의 사용량 제한 후 자동 계속**: Claude Code Desktop에서 세션 제한에 도달하면 제한 카드에서 **Auto-continue when limits reset**을 확인하고 제한이 재설정되면 앱이 중단된 턴을 다시 시도합니다.

  이번 주의 다른 기능들: **fork mode**는 대화형 세션에서 기본적으로 켜져 있으므로 Claude가 전체 대화를 상속하는 서브에이전트에 부작업을 넘길 수 있습니다. **GitLab** merge request URL은 `--worktree` 및 `claude agents` 보기와 함께 작동하며, 마켓플레이스는 bare `gitlab.com` URL을 복제합니다. 그리고 프롬프트에서 \*\*`@`\*\*를 입력하면 다른 Claude 세션을 이름으로 언급합니다.

  [Week 33 다이제스트 읽기 →](/docs/ko/whats-new/2026-w33)
</Update>

<Update label="Week 32" description="2026년 8월 3–7일" tags={["v2.1.220–v2.1.224"]}>
  **세션 간 메시징**: macOS 및 Linux에서 Claude Code 세션은 이제 서로 메시지를 보낼 수 있으므로 Claude가 한 세션에서 다른 세션으로 발견 사항이나 결정을 전달하는 대신 사용자가 다시 설명할 필요가 없습니다.

  이번 주의 다른 기능들: **self-hosted environments**는 조직이 운영하는 인프라에서 Claude Code 클라우드 세션을 실행하며, Team 및 Enterprise 플랜에서 공개 베타로 제공됩니다. **auto mode**는 8월 14일부터 Pro, Max, 및 Team 플랜의 새 세션에 대한 기본 권한 모드가 됩니다. 그리고 **VS Code extension**은 Focus view를 받습니다.

  [Week 32 다이제스트 읽기 →](/docs/ko/whats-new/2026-w32)
</Update>

<Update label="Week 30" description="2026년 7월 20–24일" tags={["v2.1.214–v2.1.219"]}>
  **Claude Opus 5**: Claude Code의 새로운 기본 Opus 모델로, 1M 토큰 컨텍스트 윈도우와 MTok당 \$10/\$50의 fast mode를 제공합니다.

  이번 주의 다른 기능들: **Claude Code Desktop**은 공개 베타에서 iOS Simulator 창을 열어서 Claude가 앱을 실행하고 사용자가 보는 동안 탭할 수 있습니다. **Claude Security plugin**은 코드베이스의 다중 에이전트 취약점 스캔을 실행하고 선택한 발견 사항을 직접 적용하는 패치로 변환합니다. 그리고 \*\*`/code-review`\*\*는 백그라운드 서브에이전트로 실행됩니다.

  [Week 30 다이제스트 읽기 →](/docs/ko/whats-new/2026-w30)
</Update>

<Update label="Week 29" description="2026년 7월 13–17일" tags={["v2.1.207–v2.1.212"]}>
  **Artifacts가 MCP 커넥터를 호출합니다**: 게시된 아티팩트는 페이지를 열 때 각 뷰어의 자신의 MCP 커넥터를 통해 라이브 데이터를 가져오고 작업을 수행할 수 있으며, 이번 주에는 공개 공유 링크, Team 및 Enterprise의 편집자 역할, 그리고 Claude Tag 세션에서 생성된 아티팩트도 추가됩니다.

  이번 주의 다른 기능들: **screen reader mode**는 시각적 터미널 인터페이스를 VoiceOver 및 NVDA와 같은 스크린 리더를 위한 일반 선형 텍스트로 대체합니다. \*\*`/fork`\*\*는 대화를 새로운 백그라운드 세션으로 복사하는 동안 사용자는 계속 작업합니다. 그리고 **auto mode**는 더 이상 Amazon Bedrock, Google Cloud의 Agent Platform, 그리고 Microsoft Foundry에서 옵트인 변수가 필요하지 않습니다.

  [Week 29 다이제스트 읽기 →](/docs/ko/whats-new/2026-w29)
</Update>

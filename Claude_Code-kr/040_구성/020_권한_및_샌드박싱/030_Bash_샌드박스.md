...
한 세션 동안 설정 파일에 쓰지 않고 샌드박스를 변경하려면 [`--settings`](/docs/ko/settings#change-a-setting-for-one-session)로 Claude Code를 시작합니다. 예를 들어 이 명령은 Claude가 차단된 명령을 샌드박스 외부에서 재시도할 수 없는 샌드박싱된 세션을 시작합니다:
